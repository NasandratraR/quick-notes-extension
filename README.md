...
Extension Chrome (Manifest V3) de prise de notes rapides, avec édition Markdown, thèmes personnalisables, groupes de notes, sommaire automatique et export/import.

## Fonctionnalités

- Éditeur Markdown avec aperçu en temps réel (mode édition / aperçu / split)
- Coloration syntaxique des blocs de code
- Organisation des notes par groupes (avec glisser-déposer)
...
└── src/                # Code de l'application, en modules ES natifs
    ...
    ├── storage.js          # Persistance via chrome.storage.local
...
Toutes les données (notes, groupes, images, préférences) sont stockées localement via `chrome.storage.local` (permissions `storage` et `unlimitedStorage`), rien n'est envoyé à un serveur distant.
